    ...
    <div class="lp-summary p-3 p-lg-4 mb-4">
      <div class="d-flex flex-column flex-lg-row align-items-lg-center justify-content-between gap-2">
        <div class="d-flex flex-wrap gap-2">
          <span class="lp-chip"><i class="bi bi-search"></i> Query: <strong id="chipQuery">feng shui</strong></span>
          <span class="lp-chip"><i class="bi bi-collection"></i> Found: <strong id="chipCount">0</strong></span>
          <span class="lp-chip"><i class="bi bi-lightning-charge"></i> Tip: try shorter keywords</span>
        </div>
      ...
      <div class="row g-3 align-items-center">
        ...
        <div class="col-6 col-lg-3">
          <select id="sortBy" class="form-select lp-select">
            <option value="relevance_desc">Sort: Relevance</option>
...
<script>
    ...
    // Relevance score: match in class/program + popularity boost
    function relevanceScore(item, q){
      const query = (q || "").trim().toLowerCase();
      if(!query) return item.popularity || 0;

      const hay = (item.class_name + " " + item.program_name + " " + item.class_id).toLowerCase();
      let score = 0;

      query.split(/\s+/).filter(Boolean).forEach(word => {
        if(hay.includes(word)) score += 50;
      });

      // enrolled classes slightly boosted
      if(item.enrolled) score += 10;

      // popularity adds gentle weight
    ...
    function render(){
      ...
      // sort
      items.sort((a,b) => {
        if(sortBy === "relevance_desc") return relevanceScore(b, q) - relevanceScore(a, q);
